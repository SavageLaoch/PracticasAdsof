...
public abstract class Dominio implements IDominio {
	...
	public double calcularFitness(IIndividuo individuo) {
		...
		for (double valor : valores.keySet()) {
			/*
			 * System.out.println("Valor: " + valor + " <-> " + "Rdo estimado: " + valor +
			 * " <-> " + "Rdo real: " + individuo.calcularExpresion(valores.get(valor)));
			 */
			if ((individuo.calcularExpresion(valor) - valores.get(valor)) <= 1
					&& (individuo.calcularExpresion(valor) - valores.get(valor)) >= -1) {
